...
def parse_messages():
    ...
    for message in get_messages():
        ...
        if text is None:
            continue
        if "FLASH SALE" in text or "SingSaver" in text or "Flash Deals" in text or "All the deals you want" in text:
            continue
        ...
        for line in lines:
            ...
            if line[0] == "✅":
                ...
                if first_tick:
                    name += line[1:]
                    cur = conn.cursor()
                    cur.execute('SELECT NAME FROM deals where NAME=?', (name,))
                    first_tick = False
                else:
                    print("debug1")
                    # extract end date
